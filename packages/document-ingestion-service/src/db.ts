...
export function createJob(job: {
  ...
  const stmt = db.prepare(`
    INSERT OR REPLACE INTO jobs (jobId, status, filePath, originalName, mimetype, size, createdAt, updatedAt)
    VALUES (@jobId, 'queued', @filePath, @originalName, @mimetype, @size, @createdAt, @updatedAt)
  `);
  stmt.run({ ...job, createdAt: now, updatedAt: now });
}
